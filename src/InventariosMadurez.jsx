...
const T = {
  ...
  L:[
    ...
    {label:"Avanzado", c:"#7C3AED",bg:"#F5F3FF",text:"#4C1D95",border:"#DDD6FE",desc:"Integración y optimización end-to-end; única fuente de verdad; trade-offs explícitos."},
    {label:"Líder",     c:"#059669",bg:"#ECFDF5",text:"#064E3B",border:"#A7F3D0",desc:"Cadena centrada en cliente; decisiones dinámicas habilitadas por IA y automatización."},
  ],
};
...
const INDUSTRY_QUESTIONS = {
  ...
  "Oil & Gas": {
    ...
    "c3": {q:"¿Qué tan bien se gestiona la obsolescencia de materiales asociada a cambios tecnológicos, equipos fuera de soporte y activos dados de baja?", label:"Obsolescencia de activos y repuestos", desc:"Obsolescencia, equipos EOL, activos", ndesc:["No se gestiona; se acumulan repuestos de equipos obsoletos sin proceso de disposición.","Se detecta la obsolescencia de forma reactiva cuando hay quiebres o equipos ya dados de baja.","Hay proceso de revisión periódica de obsolescencia con plan de disposición o reutilización.","Gestión proactiva vinculada al ciclo de vida de activos; alerta temprana de EOL de equipos.","Líder: gestión predictiva de obsolescencia con analítica de activos y recuperación de valor."], opp:"Líder: gestión predictiva de obsolescencia con analítica de activos y recuperación de valor."}, 
    ...
    <div style={{maxWidth:1060,margin:"0 auto",padding:"36px 36px 52px"}}>
      ...
      <div className="fade-up-1 hover-lift" style={{background:T.card,borderRadius:18,border:`1px solid ${T.borderSm}`,padding:"26px 28px",marginBottom:14,boxShadow:"0 2px 8px rgba(0,0,0,0.04)"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <span style={{fontSize:9,fontWeight:700,color:T.inkSoft,letterSpacing:".12em",textTransform:"uppercase",whiteSpace:"nowrap"}}>Básico</span>
          <div style={{flex:1,height:2,borderRadius:99,background:`linear-gradient(90deg,${(color+"30")},${color},${colorDk})`}}/>
          <span style={{fontSize:9,fontWeight:700,color:"#059669",letterSpacing:".12em",textTransform:"uppercase",whiteSpace:"nowrap"}}>Líder</span>
        </div>
      </div>
...
export default function App() {
  ...
  async function doGuardarProgreso(curAnswers) {
    ...
    try {
      ...
      // 1. Crear evaluacion si no existe
      if (!evalIdRef.current) {
        ...
        <div style={{flex:1,display:"flex",overflow:"hidden"}}>
          ...
          <main ref={assessScrollRef} style={{flex:1,overflow:"auto",padding:"32px 36px",position:"relative"}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:20}}>
              ...
              <button onClick={()=>guardarProgreso(answers)} disabled={saving} style={{display:"flex",alignItems:"center",gap:6,padding:"7px 18px",borderRadius:99,flexShrink:0,border:"none",background:guardarError?"#FEF2F2":savedOk?"#ECFDF5":saving?"#F0EDE9":`linear-gradient(135deg,${EC},${ECD})`,color:savedOk?"#059669":saving?"#AAA":"#fff",fontSize:12,fontWeight:700,cursor:saving?"default":"pointer",boxShadow:saving||savedOk?"none":`0 2px 12px ${EC}44`,transition:"all .2s",}}><span style={{fontSize:13}}>{guardarError?"!":saving?"⏳":savedOk?"✓":"↑"}</span><span style={{letterSpacing:".02em"}}>{guardarError?"Error":saving?"Guardando":savedOk?"Guardado":"Guardar"}</span></button>
